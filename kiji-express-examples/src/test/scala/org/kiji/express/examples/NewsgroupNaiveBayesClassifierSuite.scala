...
class NewsgroupNaiveBayesClassifierSuite extends KijiSuite {
  ...
  val calculatedWeightsInput: Seq[(String, String, Int, Int, Double)] = Seq(
  ...
  val testTableRowsInput: Seq[(EntityId, Seq[FlowCell[Int]], Seq[FlowCell[CharSequence]],
    Seq[FlowCell[CharSequence]])] = Seq(
