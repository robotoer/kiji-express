...
class TestPipe(val pipe: Pipe) extends FieldConversions with TupleConversions with TestPipeConversions {
  ...
  def assertOutputValues[A](fields: Fields, expected: Set[A])
          (implicit conv: TupleConverter[A], set: TupleSetter[Unit], flowDef: FlowDef, mode: Mode) = {
    conv.assertArityMatches(fields)

    def bf: MSet[A] = MSet[A]()

    def ef(mySet: MSet[A]): Unit = {
      val output: Set[A] = mySet.toSet

      if (expected == output) {
        logger.debug("Confirmed values on pipe!")
      } else {
        logger.debug("Mismatch in expected value for pipe and output value")

        val inExpectedNotFound: Set[A] = expected -- output
        logger.debug("Values in expected output that were not found: " + inExpectedNotFound)

        val foundButNotExpected: Set[A] = output -- expected
        logger.debug("Values found but not expected: " + foundButNotExpected)

        throw new Exception("MISMATCH!")
      }
    }

    def fn(mySet: MSet[A], tup: A): Unit = { mySet += tup }
...
class KvsJobSuite extends KijiSuite {
  ...
  def createTableAndPopulateTableAndReturnUri(
      ...
      instanceName: String = "default_%s".format(counter.incrementAndGet()) ): String = {
    ...
    class KvsFruitJob(args: Args) extends KijiJob(args) with TestPipeConversions {
      ...
      */
      KijiInput(
          ...
          // Read out everyone's favorite fruits
          tableUri = args("fruit"),
          timeRange = All,
          columns = Map(QualifiedColumnInputSpec("family", "column") -> 'slice))
          .flatMap('slice -> 'fruit) {
          ...
          .using(createPriceKeyValueStoreContext)
              .map('fruit -> 'price) { (kvs: ExpressKeyValueStore[String, Int], fruit: String) =>
                kvs(fruit) }
          .assertOutputValues(('user, 'fruit, 'city, 'price), Set(
              ("alice", "apple", "San Francisco", 1),
              ("alice", "banana", "San Francisco", 2),
              ("bob", "cantelope", "Oakland", 50),
              ("charles", "kiwi", "San Jose", 1),
              ("charles", "watermelon", "No city!!!", 2),
              ("deborah", "pear", "No city!!!", 3)
            ))
    }
    val jobTest = new KvsFruitJob(Args("--fruit " + fruitUri + " --city " + cityUri))
    jobTest.run
  }
  ...
  test("KeyValueStore works with delimited file.") {
    ...
    class KvsNbaJob(args: Args) extends KijiJob(args) with TestPipeConversions {
      ...
      IterableSource(List("Jordan", "Barkley", "Ewing", "Malone", "Magic", "Bird"), 'player)
          ...
          .assertOutputValues(('player, 'team), Set(
            ("Jordan", "Bulls"),
            ("Barkley", "Suns"),
            ("Ewing", "Knicks"),
            ("Malone", "Jazz"),
    ...
    bw.close()
    val jobTest = new KvsNbaJob(Args("--nba-csv " + csvFile.getAbsolutePath))
    jobTest.run
  ...
  test("KeyValueStore works with XML configuration file.") {
    ...
    val jobTest = new KvsNbaJob(Args("--kvs-xml " + xmlFile.getAbsolutePath))
    jobTest.run
    FileUtils.deleteDirectory(outputDir)
  }
}
